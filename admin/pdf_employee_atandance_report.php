...
if(isset($_GET['employee_id']) || $employee_id !='')
{
    ...
    $crit2 .= " and employee_id='$employee_id' ";      
}



$qry = $obj->executequery("select * from emp_attendance_entry where employee_id='$employee_id'");
foreach ($qry as $key) 
{
...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
    ...
    function Header()
    {
        global $title1,$title2,$emp_name,$year,$month_name;
         // courier 25
        ...
        $this->setY(22);
        $this->setX(60);
        $this->Cell(100,10,"Employee : ".$emp_name,0,1,'L');
...
$pdf->SetTitle($title1);
$title2 = "(EMPLOYEE ATTENDANCE REPORT LIST)";
$pdf->SetTitle($title2);
$pdf->AliasNbPages();
$pdf->AddPage('L','A4');
...
$tot = 0;
$tot_present = 0;
$res = $obj->executequery("select * from emp_attendance_entry $crit2");
...
$attendance_time = $obj->getvalfield("emp_attendance_entry","attendance_time","employee_id='$employee_id' and emp_attendance_date='$mydate'");
 $attendanceday = $obj->getvalfield("emp_attendance_entry","count(emp_attendance_date)","employee_id='$employee_id' and DATE_FORMAT(emp_attendance_date,'%m')='$aday'");
